...
//                dd($productsData);
    ...
    public function prepareDataForOrderGroupedItem($data) : array
    {
        ...
        $mappedProducts = $products->map(function (ProductGroupedProduct $product) use ($data) {
            ...
            return array_merge(
                ...
                [
                    ...
                    'base_total' => $originalProduct->getTypeInstance()->getMinimalPrice() * $product->qty * $data['quantity'],
                    ...
                    'total' => $originalProduct->getTypeInstance()->getMinimalPrice() * $product->qty * $data['quantity'],
                ]
            );
        ...
        foreach ($mappedProducts as $key => $product) {
            $mappedProducts[$key]['discount_amount'] = $proportionalDiscount + ($key == 0 ? $remainder : 0);
            $mappedProducts[$key]['base_discount_amount'] = $proportionalDiscount + ($key == 0 ? $remainder : 0);
        }
